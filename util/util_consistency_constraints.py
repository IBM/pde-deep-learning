...
def consistency_constraints_v3(y_low, y_high, chi, chi_old, kappa, iteration):
    ...
    lower = (
        alpha * chi[0] + tau * (y_low - chi[0]) + kap * (chi[0] - chi_old[0])
    )
    upper = (
        alpha * chi[1] + tau * (y_high - chi[1]) + kap * (chi[1] - chi_old[1])
    )
    return lower, upper
...
def update_consistency_constraints(data, mesh, iteration, **kwargs):
    ...
    for tile in mesh['tiles']:
        ...
        labels_min = np.amin(
            [np.amin(tile_labels) for tile_labels in data['labels'].values()])
        labels_max = np.amax(
            [np.amax(tile_labels) for tile_labels in data['labels'].values()])
        for neighbor in mesh['neighbors'][tile]:
            if not len(c_labels_new[tile][neighbor]):
                cc_chi_new[tile][neighbor] = [
                    np.full([batch_size, n_labels], labels_min),
                    np.full([batch_size, n_labels], labels_max)
                ]
            else:
                y_low = np.minimum(c_labels_new[tile][neighbor],
                                   c_labels_new[neighbor][tile])
                y_high = np.maximum(c_labels_new[tile][neighbor],
                                    c_labels_new[neighbor][tile])
                # for the first iteration, those shapes are not the same
                if cc_chi[tile][neighbor][0].shape != y_low.shape:
                    cc_chi[tile][neighbor] = [
                        np.full(y_low.shape, labels_min),
                        np.full(y_high.shape, labels_max)
                    ]

                if version == 'version 1':
                    lower, upper = consistency_constraints_v1(
                        y_low, y_high,
                        kwargs['epsilon']
                    )
                elif version == 'version 2':
                    lower, upper = consistency_constraints_v2(
                        y_low, y_high,
                        cc_chi[tile][neighbor],
                        kwargs['epsilon']
                    )
                else:  # 'version 3'
                    lower, upper = consistency_constraints_v3(
                        y_low, y_high,
                        cc_chi[tile][neighbor], cc_chi_old[tile][neighbor],
    ...
    if kwargs['do_save_cc']:
        for tile in mesh['tiles']:
            for neighbor in mesh['neighbors'][tile]:
                if tile < neighbor:
                    usb.save_consistency_constraints(cc_chi_new,
                                                     y_diff[tile][neighbor],
                                                     tile=tile,
                                                     neighbor=neighbor,
...
def get_boundary_receptor_emission_data(data, tile, neighbor, boundary,
                                        num_samples):
    """
...
def move_receptor_positions_to_boundary(c_input_tile, mesh, data, tile,
                                        neighbor, boundary, num_samples,
                                        **kwargs):
    """
        Used in update_consistency_data() below
    """
    ...
    if kwargs['add_previous_labels_to_input']:
        ...
        # 2 coords per receptor
        num_c_receptors = int((n_input - emitter_len) / 2)
    border = mesh['intersections'][tile][neighbor]
    if not len(c_receptor_pos[boundary]):
        # generate 3*num_c_receptors possible positions
        # and then choose num_c_receptors many of them
        receptor_pos = np.transpose(
            list(map(lambda x1, x2: np.linspace(x1, x2, 3 * num_c_receptors),
                     border[0], border[1])))
        selection = np.random.choice(receptor_pos.shape[0],
                                     size=num_c_receptors, replace=False)
        c_receptor_pos[boundary] = receptor_pos[selection, :]
        # normalise as in pre-processing
        # (all are np arrays, so operations are element-wise)
        c_receptor_pos[boundary] = [
            (pos - mesh['coord_mean']) / mesh['coord_std']
    ...
    pre_consistency_data = [
        list(emitters[:emitter_len]) + list(c_receptor_pos[boundary])
