...
Description:
    This module implements utility functions for saving the consistency constraints
    and performance benchmarks in the given files below. 
...
def save_consistency_constraints(cc_chi, y_diff, tile, neighbor, iteration, **kwargs):
    chi_save = [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                kwargs['random_seed'],
                kwargs['mesh_size'],
                ...
                np.average(np.abs(cc_chi[tile][neighbor][1] - cc_chi[tile][neighbor][0])),
                y_diff,
...
def save_benchmarks(tile, iteration, num_instances, num_input, num_classes,
                    test_mse, test_mae, training_start_time, mlp_times, **kwargs):
    if kwargs['do_save_benchmark']:
        benchmark = [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                     kwargs['random_seed'],
                     kwargs['mesh_size'],
                     tile,
                     num_instances,
...
def save_ml_estimates(estimates, inputs, iteration, tile, collection_mlp_estim, **kwargs):
    if kwargs['do_save_estimates'] and iteration in kwargs['iterations_to_save_estimates']:
        if kwargs['do_print_status']:
        ...
        for i, xinput in enumerate(inputs):
            save.append({'tile': tile,
                         'input': xinput,
                         'labels': estimates[i],
                         'settings': {'gamma': kwargs['cc_reg_coefficient'],
                                      'kappa': kwargs['kappa'],
                                      'iteration': iteration,
