...
Description:
    This module implements utility functions to decompose the main domain into smaller sub-somains, 
    as well as methods to place receptors and get the emitters. The method get_utilities() at the end
    retrieves all utility information for the specified parameters. 
    
    This module fetches utility information for a run from the database. If this information is not available,
    it will be generated and stored as a new entry in a collection 'util' of a MongoDB database called 
    'db_air_quality' at port 27018.
...
def select_links_in_area(links_dict, bounding_box):
    ...
    for key, link in links_dict.items():
        ...
        lat2 = link[1][0]
        lon2 = link[1][1]
        if (coord_min[0] <= lat1 <= coord_max[0] and coord_min[1] <= lon1 <= coord_max[1]) \
                or (coord_min[0] <= lat2 <= coord_max[0] and coord_min[1] <= lon2 <= coord_max[1]):
...
def decompose_domain(coord_min, coord_max, links_in_area):
    """
    select rectangular areas with a maximum of 20 links
    shifting version discussed with Philipp
    this version DOES handle links that intersect areas
    ...
    # Here i could not think of a better way, i played around to get a 3x4 grid...
    index = 100
    ...
    for i in range(index - 1):
        ...
        for link, link_coords in links_in_area.items():
            link_in_area = is_link_in_area(link_coords, coord_min_temp, coord_max_temp)
    ...
    for key in box_sub_dict:
        ...
        for i in range(0, index - 1):
            ...
            for link in sub_dict['links']:
                link_in_area = is_link_in_area(links_in_area[link], coord_min_temp, coord_max_temp)
                if link_in_area is True and sub_domain_id not in links_dict[link]:
                    counter += 1
                    domain_dict[sub_domain_id]['links'].append(link)
                    links_dict[link].append(sub_domain_id)

            if nb_links_per_area <= counter <= 20:
                domain_dict[sub_domain_id]['coord'] = [list(lon_lat_min), list(coord_max_temp)]
...
def decompose_domain_new(coord_min, coord_max, links_in_area, max_links_per_sub_domain=20):
    """
    first draft of an attempt at improving the decompose_domain method
    INCOMPLETE
    ...
    :param coord_max:
    :param links_in_area:
    :param max_links_per_sub_domain:
    :return: links_dict: {(site, next_site): [sub_domain_id], ...}
             domain_dict: {sub_domain_id: {'coord': [[lat, lon], [lat, lon]],
                                 'links': [(site, next_site), ... ]}}
    """
    max_links = np.ceil(len(links_in_area) / max_links_per_sub_domain)

    links_dict = defaultdict(list)
    domain_dict = {}
    grid_res = 100
    lat_grid = np.linspace(coord_min[0], coord_max[0], grid_res)
    lon_grid = np.linspace(coord_min[1], coord_max[1], grid_res)

    links = []
    sub_domain_id = 1
    sub_domain_min = [lat_grid[0], lon_grid[0]]
    sub_domain_max = [lat_grid[1], lon_grid[1]]  # going to be replaced
    i = 0
    j = 0
    for lat in lat_grid[1:]:
        ...
        for lon in lon_grid[1:]:
            ...
            new_sub_domain_max = [lat, lon]
            links_in_sub_domain = [tuple(sorted(link)) for link, link_coords in links_in_area.items()
                                   if is_link_in_area(link_coords, sub_domain_min, new_sub_domain_max)]
            if len(links) + len(links_in_sub_domain) <= max_links:
                links += links_in_sub_domain
                sub_domain_max = new_sub_domain_max
            else:
                # save previous tile
                for link in links:
                    links_dict[link].append(sub_domain_id)
                domain_dict[sub_domain_id] = {'coord': [sub_domain_min, sub_domain_max],
                                              'links': links}
                # set up new tile
                i = 0
                j = 0
                ...
                links = links_in_sub_domain
                if lon < coord_max[1]:
                    sub_domain_min = [sub_domain_min[0], sub_domain_max[1]]
                else:
                    sub_domain_min = [sub_domain_max[0], coord_min[1]]
                    break
...
def get_neighboring_domains(domain_dict):
    ...
    def get_edges(min_max_list):
        corners = list(min_max_list)  # decorate with list to avoid changing domain_dict
            ...
            if other_sub_domain_id != sub_domain_id:
                ...
                for edge in edges:
                    for other_edge in other_edges:
                        for point in edge:
                            if distance_point_from_line(point, other_edge) == 0 and point not in intersection:
                                intersection.append(list(point))
                        for point in other_edge:
                            if distance_point_from_line(point, edge) == 0 and point not in intersection:
                                intersection.append(list(point))
                if len(intersection) > 1:
                    neighbors[sub_domain_id].append(other_sub_domain_id)
                    intersections[sub_domain_id][other_sub_domain_id] = intersection
...
def create_contour(bounding_box, links, interval):
    """
    :param bounding_box:
    :param links:
    :param interval:
    ...
    """
    step_lat = geopy.distance.vincenty((bounding_box[0][0], bounding_box[0][1]),
                                       (bounding_box[1][0], bounding_box[0][1])).meters
    step_lon = geopy.distance.vincenty((bounding_box[0][0], bounding_box[0][1]),
                                       (bounding_box[0][0], bounding_box[1][1])).meters
    # have contour not start on boundary, but slightly off
    eps = [(bounding_box[1][0] - bounding_box[0][0]) / (10 * (step_lat / interval + 1)),
           (bounding_box[1][1] - bounding_box[0][1]) / (10 * (step_lon / interval + 1))]
    ...
    for lat in list_lat:
        for lon in list_lon:
            dist = min([distance_point_from_line([lat, lon], link) for link in links])
            # round to accuracy of interval
            dist = interval * int(dist / interval)
            contour_dict[dist].append([lat, lon])

    return contour_dict


def get_receptors(domain_dict, links_in_area, contour_interval=25, include_stations=True):
    """
        receptors are randomly allocated!
    :param domain_dict:
    :param links_in_area:
    ...
    for sub_domain_id, sub_domain in domain_dict.items():
        ...
        contour_dict = create_contour(bounding_box, links, interval=contour_interval)

        # continue if there are less than 20 possible receptor positions available
        # this also removes this box from the domain_dict.
        contour_num = {dist: len(ct) for dist, ct in contour_dict.items() if dist != 0}

        if include_stations:
            receptor_stations = [s for s in stations.values() if is_point_in_area(s, bounding_box)]
        else:
            receptor_stations = []
        ...
        while sum(receptor_count) < num_receptors:
            for i, spacing in enumerate(receptor_spacings):
                receptor_count[i] += 1 if receptor_count[i] < contour_num[spacing] else 0
                if sum(receptor_count) == num_receptors:
                    break

        for d, distance in enumerate(receptor_spacings):
            receptor_contour = contour_dict[distance]
            # indices = [int(i) for i in np.linspace(0, len(receptor_contour) - 1,
            #                                        min(receptor_count[d], len(receptor_contour)))]
            indices = np.random.choice(range(len(receptor_contour)), receptor_count[d], replace=False)
...
def get_emitters(domain_dict, links_in_area):
    ...
    for sub_domain_id, sub_domain in domain_dict.items():
        for link in sub_domain['links']:
            ...
            emitters_dict[sub_domain_id].append([lat_start, lon_start, lat_end, lon_end])
            emitters_dict_cart[sub_domain_id].append(list(utm_coord_start[:2] + utm_coord_end[:2]))
...
def normalise(emitters_dict_cart, receptors_dict_cart):
    ...
    # normalise
    norm_emitters = {key: [list(map(operator.sub, cart, [x_min, y_min, x_min, y_min]))
...
def get_utilities(collection_utilities, collection_traffic, ref_date,
                  run_tag, bounding_box, contour_distance, include_stations):
    """
        Returns utility information from the database based on run_tag. If no information
        is available, a new dictionary is generated and added to the utilities database.
    :param collection_utilities:
    :param collection_traffic:
    :param ref_date: date of reference for which the traffic links are collected.
    :param run_tag:
    :param bounding_box:
    ...
    util = uda.get_utilities_from_collection(collection_utilities, run_tag)
    if len(util) > 0:
        ...
        traffic_sources = collection_traffic.find({'date': ref_date.strftime('%Y-%m-%d %H:%M:%S')})
        source_links = {}
        for source in traffic_sources:
            start = source['site']
            stop = source['next_site']
            if (start, stop) not in source_links and (stop, start) not in source_links:
                source_links[(start, stop)] = [source['site_coord'], source['next_site_coord']]

        util['bounding_box'] = bounding_box
        util['links_in_area'] = select_links_in_area(source_links, bounding_box)
        print('Decompose domain ...')
        util['links_dict'], util['domain_dict'] = decompose_domain(bounding_box['coord_min'],
                                                                   bounding_box['coord_max'],
                                                                   util['links_in_area'])
        util['domain_neighbors'], util['intersections'] = get_neighboring_domains(util['domain_dict'])
        print('Place receptors ... ')
        util['receptors_dict'], receptors_dict_cart = get_receptors(util['domain_dict'],
                                                                    util['links_in_area'],
                                                                    contour_distance,
                                                                    include_stations)
        ...
        for sub_domain_id, receptor_list in util['receptors_dict'].items():
            for i, receptor in enumerate(receptor_list):
                util['receptors_index'][tuple(receptor)] = {'run_tag': run_tag,
                                                            'domain': sub_domain_id,
                                                            'index': i}
        util['emitters_dict'], emitters_dict_cart = get_emitters(util['domain_dict'],
                                                                 util['links_in_area'])
        print('Normalize emitter and receptor coordinates.')
        util['norm_emitters'], util['norm_receptors'] = normalise(emitters_dict_cart,
                                                                  receptors_dict_cart)
