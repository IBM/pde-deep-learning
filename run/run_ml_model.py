...
def get_parameters():
    param = {  # data parameters
        'mesh_size': 2,  # number of sub-domains
        ...
        'add_previous_labels_to_input': False,  # ToDo: allow True in update of consistency constraint data
        # consistency constraints
        'use_consistency_constraints': False,
        'cc_reg_coefficient': 1,  # lambda
        'kappa': 0.5,
        'epsilon': 0.1,
        'cc_update_version': 'version 3',  # check util.util_consistency_constraints
        # saving of output
        ...
        # for reproducibility
        'random_seed': None  # None or (int) < 2147483648. If None, it's taken at random
    }
    return param
...
def main():
    ...
    if param['do_print_status']:
        print(f'Using a mesh with {param["mesh_size"]} tile(s).')
        print(f'Hidden layers: {param["num_hidden_layers"]}\t nodes: {param["num_nodes"]}\t')
        print(f'Lambda: {param["cc_reg_coefficient"]}\t kappa: {param["kappa"]}\t epsilon: {param["epsilon"]}\n')
        print('Loading data ...')
    ...
    for iteration in range(param['num_iterations']):
        mlp_times = umm.run_recursion_cycle(data, mesh, iteration, collections['pred'], **param)
        if param['use_consistency_constraints']:
            ucc.update_consistency_constraints(data, mesh, iteration, **param)
        if param['do_print_status']:
            print(f'Average MLP run time for all data of one tile: {np.average(mlp_times):.3f}s')
            print(f'Standard Deviation: {np.std(mlp_times):.3f}s')
            print('')
